...
public class Node extends UnicastRemoteObject implements NodeInterface {
	...
	public Message onMutexRequestReceived(Message message) throws RemoteException {
		Message reply = new Message(nodeID, nodename, port);
		reply.setClock(message.getClock());
		reply.setNodeName(nodename);
		reply.setPort(port);

		boolean allowed = mutex.onMutexRequestReceived(message);
		reply.setAcknowledged(allowed);
